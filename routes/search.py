...
def search_by_text(current_user):
    try:
        ...
        all_results = ai_service.search_similar(query_embedding, k=3)  # Lấy nhiều kết quả hơn
...
def search_by_image(current_user):
    try:
        ...
        # Tìm ảnh tương tự
        all_results = ai_service.search_similar(query_embedding, k=2)  # Lấy nhiều kết quả hơn
        if not all_results:
            return jsonify({
                'results': [],
                'pagination': {
